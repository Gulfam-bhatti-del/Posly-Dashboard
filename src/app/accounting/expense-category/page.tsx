...
export default function ExpenseCategoriesComponent() {
  const [isExpenseDialogOpen, setIsExpenseDialogOpen] = useState(false)
  const [isEditDialogOpen, setIsEditDialogOpen] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [editingExpense, setEditingExpense] = useState<Expense_Deposit_Category | null>(null)
  const [notification, setNotification] = useState<{
    type: "success" | "error" | null
    message: string
  }>({ type: null, message: "" })

  const [expenseForm, setExpenseForm] = useState<Omit<Expense_Deposit_Category, "id" | "created_at">>({
    title: "",
  })

  const [expenses, setExpenses] = useState<Expense_Deposit_Category[]>([])
  const [searchTerm, setSearchTerm] = useState("")
  const [entriesPerPage, setEntriesPerPage] = useState("10")
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [deleteLoading, setDeleteLoading] = useState(false)
  const [expenseToDelete, setExpenseToDelete] = useState<Expense_Deposit_Category | null>(null)
  ...
  const fetchExpenses = async () => {
    try {
      ...
      showNotification("error", "Failed to load expense categories")
    }
  }
  ...
  const handleAddExpense = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("expense_categories").insert([expenseForm])

      if (error) {
        console.error("Insert error:", error)
        showNotification("error", `Failed to add category: ${error.message}`)
      } else {
        showNotification("success", "Expense category added successfully")
        setIsExpenseDialogOpen(false)
        setExpenseForm({ title: "" })
        await fetchExpenses()
      }
    } catch (err) {
  ...
  const handleEditExpense = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase
        .from("expense_categories")
        .update({ title: expenseForm.title })
      ...
      if (error) {
        ...
        showNotification("success", "Expense category updated successfully")
  ...
  const handleDeleteExpense = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        showNotification("success", "Expense category deleted successfully")
  ...
  const openEditDialog = (expense: Expense_Deposit_Category) => {
    setEditingExpense(expense)
    setExpenseForm({ title: expense.title })
    setIsEditDialogOpen(true)
  }

  const openDeleteDialog = (expense: Expense_Deposit_Category) => {
  ...
  const displayedExpenses =
    entriesPerPage === "All" ? filteredExpenses : filteredExpenses.slice(0, Number.parseInt(entriesPerPage))
          ...
          <CardContent className="p-0">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t bg-white">
              <div className="text-sm text-gray-600">
                Showing {displayedExpenses.length > 0 ? 1 : 0} to {displayedExpenses.length} of{" "}
            ...
            <form onSubmit={handleAddExpense} className="space-y-4">
              <div className="space-y-4">
                ...
                <Input
                  ...
                  onChange={(e) => handleExpenseInputChange("title", e.target.value)}
                  required
