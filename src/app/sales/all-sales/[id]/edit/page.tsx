...
const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setMessage("Redirecting to sales list...");
      }, 1000);
    } catch (error) {
      setMessage("Error updating sale. Please try again.");
    }

    setLoading(false);

    router.push('/sales/all-sales')
  };

  return (
    <div className="container mx-auto p-4 max-w-6xl">
      <h1 className="text-3xl font-bold mb-2">Edit Sale</h1>
          ...
          <CardFooter className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button
                type="submit"
                disabled={
                  loading || !form.ref || !form.customer || !form.warehouse
                }
                onClick={() => toast}
              >
                {loading ? (
                ...
                {loading ? "Updating Purchase..." : "Update Purchase"}
              </Button>
              <ToastContainer />
              <Button
                type="button"
